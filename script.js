...
function renderHomeGuide() {
  const guide = document.getElementById("home-board-guide");
  const actions = document.getElementById("home-board-actions");
  if (!guide || !actions) return;
  const user = getCurrentUser();
  if (user) {
    guide.textContent = "회원게시판에 글을 작성할 수 있습니다.";
    actions.innerHTML = `
      <a class="btn" href="board.html">회원게시판으로 이동</a>
    `;
  } else {
    guide.textContent = "로그인 후 회원게시판에서 소식을 남길 수 있습니다.";
    actions.innerHTML = `
      <a class="btn-outline" href="login.html">로그인</a>
      <a class="btn" href="register.html">회원가입</a>
    `;
